...
import { SpinnerIcon, UploadIcon, FileIcon } from '../components/Icons';
...
export default function ConfigureJobView() {
  ...
  const handleConfigConfirm = async (startPage: number, isFull: boolean, endPage?: number, voiceId?: string) => {
    if (!jobId) return;
    ...
    try {
      ...
      if (isFull) {
        ...
        await startJobMutation.mutateAsync({ jobId, start: startPage, end: finalEnd, voiceId });
        await refreshUser();
    ...
    return (
      <div className="max-w-xl w-full mx-auto bg-white rounded-[3rem] shadow-2xl p-10 border border-slate-100 animate-fade-in-up">
        <div className="text-center mb-10">
          ...
          <p className="text-slate-500 font-medium">Your document has been removed from our server (5-day limit).</p>
        </div>
        ...
        <div className="bg-red-50 border border-red-100 rounded-2xl p-6 mb-8 text-center">
          <p className="text-red-700 font-bold text-sm">
            ...
            You must upload the exact same document to avoid processing conflicts and audio misalignment.
          </p>
        </div>
        ...
        {!recoveryFile ? (
          ...
          >
            ...
            <p className="font-black text-slate-900 uppercase text-xs tracking-widest">Select Original Document</p>
            <input type="file" ref={fileInputRef} className="hidden" accept=".pdf,.epub,.txt,.docx" onChange={(e) => e.target.files && setRecoveryFile(e.target.files[0])} />
          </div>
        ) : (
